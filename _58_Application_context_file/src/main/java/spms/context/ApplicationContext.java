...
// applictioin-context.properties 파일을 읽어서 필요한 객체를 준비/저장
public class ApplicationContext {
	// proterties파일이 key=value 구조로 되어 있기 때문에 Map을 사용한다.
	...
	public ApplicationContext(String properiesPath) throws Exception{
		Properties props = new Properties();
		props.load(new FileReader(properiesPath));
		
		prepareObjects(props);
		injectDependency();
	}
	
	// properties 파일을 읽어서 객체를 찾아서 참조하고 생성한다.
	...
	private void callSetter(Object obj) throws Exception{
		...
		for(Method m: ms) {
			if(m.getName().startsWith("set")) {
				// setter의 첫번째 파라미터(즉, 주입해야할 객체 타입) 정보
				dependency = findObjectByType(m.getParameterTypes()[0]);
				
				// 해당 파라미터에 해당하는 객체를 찾아서 반환 받았으면,
				// setter를 호출하고 첫번째 파라미터에 객체를 넣는다.
				if(dependency != null) {
