...
public class DBconnectionPool {
	...
	// ArrayList대신 Vector를 변경한다.
	// ArrayList와 Vector는 동일한 구조이지만, Vector 멀티스레드 동기화 처리가 되어있다.
	...
	public Connection getConnection() throws Exception{
		//현재 사용하지 않고 있는 Connection객체가 존재한다면
		if(connList.size() > 0) {
		...
		// 뜻이므로 새롭게 서버와 연결된 Connection객체를 생성해서 return 한다.
